...
export default function OrderEditModal({
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (platformConfig?.editableFields?.includes("tracking_number") && values.tracking_number) {
      ...
      if (platformConfig?.editableFields?.includes("wc_status") && values.wc_status) {
        updateData.wc_status = values.wc_status;
      }
      if (platformConfig?.editableFields?.includes("wm_status") && values.wm_status) {
        updateData.wm_status = values.wm_status;
      }
      if (platformConfig?.editableFields?.includes("status") && values.status) {
        updateData.status = values.status;
      }
      if (platformConfig?.editableFields?.includes("kits") && values.kits && values.kits.length > 0) {
      ...
      Swal.fire({
        icon: "error",
        title: "Update Failed",
        text: error.response?.data?.message || error.message || "Failed to update order",
        toast: true,
        position: "top-end",
        showConfirmButton: false,
        timer: 4000,
