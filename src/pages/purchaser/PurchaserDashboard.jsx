...
export default function PurchaserDashboard() {
  ...
  const {
    ...
    for (const d of rows) {
      const rawStatus = String(d?.status ?? "").trim();
      if (rawStatus && lower(rawStatus) === "purchased") {
        purchasedCount += 1;
      }
      ...
      const actual =
        num(d?.total_actual_cost) ||
        (num(d?.sellers_price) + num(d?.shipping_charges) + num(d?.taxes));
      const s = Number.isFinite(+direct) ? +direct : target - actual;
      if (Number.isFinite(s)) savings += s;
    ...
    return {
      ...
      avgResponseMs: responseCount
        ? Math.round(responseSumMs / responseCount)
        : null,
  ...
  return (
    <div className="mt-4 p-0 md:p-[1.25rem] rounded-[16px] sm:shadow-[0_12px_28px_rgba(15,23,42,0.06)] sm:border border-[#e6edff] bg-none md:bg-[linear-gradient(135deg,#f8fbff,#eef4ff)]">
      ...
      {/* Removed: By Status / Top Markets / Top Sellers breakdowns */}
