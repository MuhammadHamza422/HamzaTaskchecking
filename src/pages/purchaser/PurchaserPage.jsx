...
export default function PurchaserPage() {
  ...
  // ── Tab keys: re-ordered and "returned" removed
  const TAB_KEYS = ["dashboard", "pending", "all", "listings"];
  ...
  const getInitialActiveTab = () => {
    ...
    if (savedTab && TAB_KEYS.includes(savedTab)) return savedTab;
    return "dashboard";
  };
  ...
  const serverParams = useMemo(() => {
    const params = {};
    // Scope: non-admins default to "mine"
    if (isAdmin) {
    ...
    // Status: apply ONLY on "All Listings"
    if (activeTab === "listings" && statusFilter) {
  ...
  const fetchAssigned = useCallback(async () => {
    ...
    try {
      ...
      // Clone and adjust params as needed
      const params = { ...serverParams };

      // "All Assigned" must ignore status
      if (activeTab === "all") delete params.status;

      // On "All Listings": admins see everyone; non-admins keep mine=true
      if (activeTab === "listings") {
        if (isAdmin) delete params.mine;
      }

      const { data } = await apiClient.get(endpoint, {
        params: { ...params, page, limit, sort: "-createdAt" },
      });
  ...
  }, [activeTab, serverParams, page, limit, isAdmin]);
  ...
  return (
    ...
    >
        ...
        <Row gutter={[8, 8]}>
          <Col xs={24} sm={12} md={6} lg={5}>
            <Select
              ...
              size={screens.xs ? "middle" : "large"}
              // NOTE: Status is ignored on "All Assigned", applied on "All Listings"
            >
              {[
                "Assigned",
                "Offer",
      ...
      >
        ...
        {/* 3) All Assigned (ignores Status filter) */}
        ...
        {/* 4) All Listings (applies Status filter; admins see everyone) */}
        <TabPane tab="All Listings" key="listings">
          <Table
