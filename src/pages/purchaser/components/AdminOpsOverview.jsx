...
//   value:
//     "text-base md:text-lg font-semibold text-slate-900 tabular-nums truncate leading-tight",
//   cardBase:
//     // REVERT: clip children so the top accent respects rounded corners.
...
// /** DB: purchase_efficiency === "Savings". Fallback: Target − Actual. */
// const getSavings = (r) => {
...
//   return createPortal(
//     <div
//       className="fixed max-w-[260px] rounded-md border border-slate-200 bg-white px-3 py-2 text-[11px] text-slate-700 shadow-lg z-[100000]"
...
// const KPI = ({ icon: Icon, label, value, tone = "blue", bgGradient = false }) => {
//   const p = PALETTES[tone] || PALETTES.blue;
//   return (
...
//               {row.key}
//             </div>
//             <div className="w-24 text-right text-[13px] text-slate-900 tabular-nums">
//               {row.total}
//             </div>
//           </div>
//         ))}
...
//   const moneyFmt = (v) =>
//     new Intl.NumberFormat(undefined, { style: "currency", currency: "USD" }).format(num(v));
...
//           <KPI icon={DollarSign} label="Savings" value={moneyFmt(counts.savings ?? 0)} tone={listingTones[7]} bgGradient />
//         </div>
//       </Section>

//       {/* TRACKING OVERVIEW — NO TOOLTIP */}
...
const theme = {
  ...
  value:
    "text-base md:text-lg font-semibold text-slate-900 tabular-nums truncate leading-tight",
  cardBase:
    // clip so the top accent respects rounded corners
    "relative overflow-hidden rounded-xl transition-all duration-150 hover:shadow-md hover:bg-slate-50/40",
  chipWrap: "grid h-8 w-8 place-items-center rounded-md",
};
...
/** Show "- $1,234.56" or "$1,234.56" with comma separators */
...
const signClass = (v) => (num(v) < 0 ? "text-red-600" : "text-emerald-600");
...
const KPI = ({
  ...
  const vClass = money && colorBySign ? signClass(value) : "";
  return (
    <Card tone={tone} bgGradient={bgGradient}>
      <div className="p-3 h-full flex items-center gap-3">
        <span className={`${theme.chipWrap} ${p.chip}`}>
        ...
        <div className="flex-1 min-w-0">
          <div className={theme.label}>{label}</div>
          <div className={`${theme.value} ${vClass}`}>{display}</div>
        </div>
      </div>
    </Card>
  );
    ...
    <Card tone={tone} bgGradient={bgGradient}>
      ...
      <div className="divide-y divide-transparent">
        {items.map((row) => {
          ...
          const vClass = money && colorBySign ? signClass(raw) : "text-slate-900";
          return (
            <div
              key={row.key}
              className="px-3 py-2 flex items-center rounded-lg hover:bg-slate-50 transition-colors"
            >
              ...
              <div className={`w-24 text-right text-[13px] tabular-nums ${vClass}`}>
                {display}
              </div>
            </div>
          );
    ...
    >
      ...
      <Section icon={ClipboardList} title="Listing Overview" tone="blue">
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-8 gap-3 items-stretch">
          ...
          <KPI icon={PauseCircle} label="On Hold" value={counts.onHold ?? 0} tone={listingTones[5]} bgGradient />
          <KPI icon={Store} label="In Auction" value={counts.inAuction ?? 0} tone={listingTones[6]} bgGradient />
          {/* Savings: money format + color by sign */}
          <KPI
            icon={DollarSign}
            label="Savings"
