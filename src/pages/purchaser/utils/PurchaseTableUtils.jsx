...
//   // order-level rollups (same as SourcerPage)
//   const orderTarget =
//     Number.isFinite(Number(order?.target_total_cost))
//       ? safeNum(order.target_total_cost)
//       : (rows.reduce(
//           (s, it) => s + safeNum(it.quantity_needed || 1) * safeNum(it.target_cost_per_unit), 0
//         ));
//   const orderActual =
...
//   // per-item helpers (match SourcerPage)
...
//                       color: "#1677ff", fontWeight: 600, cursor: "pointer",
//                     }}
//                   >
//                     {label}
//                   </button>
...
//             width: 80,
//             align: "center",
//             render: (v) => <span style={{ fontVariantNumeric: "tabular-nums" }}>{safeNum(v)}</span>,
//           },
//           {
//             title: "Target price / unit",
//             dataIndex: "target_cost_per_unit",
...
//               const cls = val >= 0 ? { color: "#16a34a", fontWeight: 700 } : { color: "#ef4444", fontWeight: 700 };
//               return <span style={cls} className="tabular-nums">{fmt(val)}</span>;
//             },
//           },
//           {
//             title: "Saving / SKU",
...
/* ---------------- Tooltip helpers (match SourcerPage) ---------------- */
const tipCommon = {
...
export function ExpandedItemsTable({ order, onOpen }) {
  ...
  return (
    <div className="overflow-x-auto">{/* no extra borders/padding to avoid gaps */}
      <Table
        rowKey={(r) => r._id || r.id || `${r.sku}-${r.product_name || r.name || "item"}`}
        size="small"
        bordered={false}
