...
import { Table, Input, Typography, Empty, Pagination, Drawer, Tag, Spin } from "antd";
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      ...
      <div className="bg-white rounded-lg">
        <Table
          ...
          locale={{
            emptyText: (
              <Empty description="No merged products found" className="py-12 text-xl font-semibold text-gray-500" />
            ),
          }}
        ...
        {!selectedRow ? (
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="text-base font-semibold">{selectedRow.pro_title}</div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                ...
                <div>
                  <Tag color={selectedRow.cnd_code === "R" ? "green" : selectedRow.cnd_code === "N" ? "blue" : "default"}>
              ...
              <div>
                ...
                <div className="text-gray-700">{selectedRow.storage_code}</div>
              </div>
              <div>
                <Text type="secondary" className="block text-xs">
                  Color
            ...
            <div className="grid grid-cols-3 gap-3 text-sm">
              ...
              <div>
                ...
                <div className="font-semibold">${Number(selectedRow.price || 0).toFixed(2)}</div>
              </div>
              <div>
                <Text type="secondary" className="block text-xs">
                  Sale Price
                </Text>
                <div className="font-semibold text-green-600">${Number(selectedRow.sale_price || 0).toFixed(2)}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <div className="text-gray-700">
                  {[selectedRow?.user?.firstName, selectedRow?.user?.lastName].filter(Boolean).join(" ") || "-"}
                </div>
              </div>
              <div>
                <Text type="secondary" className="block text-xs">
                ...
                <div className="text-gray-700">{new Date(selectedRow.createdAt).toLocaleString()}</div>
              </div>
              <div>
                <Text type="secondary" className="block text-xs">
                  Updated At
                </Text>
                <div className="text-gray-700">{new Date(selectedRow.updatedAt).toLocaleString()}</div>
              </div>
            </div>

            {Array.isArray(selectedRow.sources) && selectedRow.sources.length > 0 && (
              <div>
                ...
                <div className="max-h-72 overflow-auto border rounded-md p-2 space-y-2">
                  {selectedRow.sources.map((sp, idx) => (
                    <div key={idx} className="border rounded p-2">
                      <div className="font-medium">{sp.pro_title || sp.title}</div>
                      <div className="text-xs text-gray-600">SKU: {sp.sku}</div>
                    </div>
                  ))}
                </div>
              </div>
