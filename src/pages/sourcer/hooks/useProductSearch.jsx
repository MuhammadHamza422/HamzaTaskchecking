...
export default function useProductSearch(typeCode) {
  ...
  // prevent stale/out-of-order responses from racing
  const reqSeq = useRef(0);

  // keep the latest term without re-render loops
  ...
  // react to TYPE changes only (read current term from ref)
  useEffect(() => {
    debouncedSearch.cancel();
    const currentTerm = termRef.current;
    if (currentTerm.trim()) {
      fetchProducts(currentTerm);
    } else {
  ...
  }, [typeCode, fetchInitialProducts, fetchProducts]);

  return { loading, term, products, debouncedSearch, fetchInitialProducts, fetchProducts, setTerm };
}
