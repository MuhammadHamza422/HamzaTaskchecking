...
// //   // fine-grained perms from /api/v1/role/all
...
// //   // const [canImport, setCanImport] = useState(false); // wire if you need Import on dashboard
...
// //           // setCanImport( /* menu.includes('import csv') || */ !!createOrder );
// //           setRolesLoaded(true);
// //         }
// //       } catch (err) {
...
// //   /* ------------------------ admin sourcer options ----------------------- */
// //   const loadSourcerOptions = useCallback(async () => {
// //     // You may decide to hide this when myRequests is OFF; keeping fetch behind admin + canViewMyRequests
// //     if (!canViewMyRequests) return;
// //     // admin-ness from name is okay only for this dropdown; still no data shown if myRequests is OFF
...
// //     let cancelled = false;
// //     (async () => {
// //       if (!rolesLoaded) return; // wait for perms
// //       setLoading(true);
// //       try {
// //         if (!canViewMyRequests) {
// //           // STRICT: no data load at all
...
// //             setOrders([]);
// //           } else {
// //             // Admin can pick a sourcer to view — still respects canViewMyRequests
// //             const isAdminByName = roleName === "admin";
...
// //   const totalBaseline = useMemo(() => enriched.reduce((s, r) => s + num(r.target_total), 0), [enriched]);
// //   const totalSavings = useMemo(() => enriched.reduce((s, r) => s + num(r.savings_dollar), 0), [enriched]);
// //   const requestsPending = useMemo(() => enriched.filter((r) => r.status === "Pending").length, [enriched]);
// //   const requestsPurchased = useMemo(() => enriched.filter((r) => r.status === "Purchased").length, [enriched]);

// //   const filtered = useMemo(() => {
// //     return enriched.filter((order) => {
// //       const items = order.items || [];
// //       const matchesProductId =
// //         !filters.productId || items.some((i) => String(i._id || i.id || i.product_id) === String(filters.productId));
// //       const matchesProductText =
// //         !filters.productText ||
// //         items.some((i) => (i.name || i.product_name || "").toLowerCase().includes((filters.productText || "").toLowerCase()));
// //       const matchesStatus = !filters.status || order.status === filters.status;
// //       const created = order.created_at || order.createdAt || order.created_on;
// //       const matchesDate =
// //         !filters.dateRange ||
...
// //             itemTable={makeItemsTable}
// //             statusPill={statusPill}
// //             // buildEditUrl={(id) => `/sourcing/orders/${id}/edit`} // uncomment if your route differs
// //           />
// //         </motion.div>
// //       </div>
// //     </div>
...
//   // Apply filters (including default current-month range)
//   const filtered = useMemo(() => {
//     return enriched.filter((order) => {
//       const items = order.items || [];
//       const matchesProductId =
...
//   // 🔹 Metrics now computed from FILTERED (current-month by default)
//   const totalBaseline = useMemo(() => filtered.reduce((s, r) => s + num(r.target_total), 0), [filtered]);
//   const totalSavings = useMemo(() => filtered.reduce((s, r) => s + num(r.savings_dollar), 0), [filtered]);
//   const requestsPending = useMemo(() => filtered.filter((r) => r.status === "Pending").length, [filtered]);
//   const requestsPurchased = useMemo(() => filtered.filter((r) => r.status === "Purchased").length, [filtered]);
...
//                 <RangePicker
//                   className="w-full"
//                   // 🔹 Controlled to current month by default
...
//         {/* Recent requests table (month-scoped by default) */}
//         <motion.div
//           initial={{ opacity: 0, y: 8 }}
//           animate={{ opacity: 1, y: 0 }}
//           transition={{ delay: 0.15 }}
