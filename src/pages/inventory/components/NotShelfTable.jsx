  ...
  return (
    <div className="overflow-x-auto">
      <table className="min-w-full bg-white rounded-lg border">
        ...
        <tbody>
          {items.map((item, idx) => (
            <tr key={item.id} className={`border-t ${idx % 2 !== 0 ? "bg-gray-50" : "bg-white"}`}>
              <td className="px-4 py-3 text-sm font-medium">{item.productTitle}</td>
              ...
              <td className="px-4 py-3">
                  ...
                  <div className="flex items-center rounded-lg overflow-hidden border border-gray-200 shadow-sm bg-gray-50 h-9">
                    <button
                      ...
                      onClick={() => {
                        ...
                        const baseQty = currentPending ? currentPending.newQty : Number(item.quantity);
                    ...
                    <input
                      value={pendingQtyChanges.has(item.id) ? pendingQtyChanges.get(item.id).newQty : item.quantity || ""}
                      onChange={(e) => handleQuantityInputChange(item.id, e.target.value)}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          const pendingChange = pendingQtyChanges.get(item.id);
                          if (pendingChange) {
                            const key = pendingChange.newQty > pendingChange.currentQty ? "added" : "removed";
                ...
                {pendingQtyChanges.has(item.id) && (
                  <div className="fixed left-0 bottom-0 w-full p-5 bg-white flex flex-col sm:flex-row sm:items-center gap-y-2 justify-between duration-300 ease-in-out">
                    <h2 className="text-xl sm:text-2xl font-bold">{item.productTitle}</h2>
                    <div className="flex items-center max-sm:justify-end space-x-2">
                      <p className="text-sm font-medium text-blue-700">New qty: {pendingQtyChanges.get(item.id)?.newQty}</p>
                      <button
                        onClick={() => {
                          const pendingChange = pendingQtyChanges.get(item.id);
                          if (pendingChange) {
