...
const MAX_PHOTOS = 10; // Updated per API guide: 1-10 photos allowed
const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB
const ACCEPTED_FORMATS = ["image/jpeg", "image/png", "image/webp"];

export default function ShippingPhotoUploadStep({ 
  photos = [], 
  onPhotosChange, 
  onComplete, 
  canComplete, 
  ...
  // Auto-open camera on first mount if requested
  useEffect(() => {
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-2 md:p-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm md:text-base text-gray-600">
            Upload up to {MAX_PHOTOS} photos. At least 1 photo is required to complete shipping.
          </p>
        </div>
          ...
          {canAddMore && (
            <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
              <motion.button
                ...
                className="flex-1 flex items-center justify-center gap-2 py-3 md:py-4 px-4 md:px-6 bg-purple-600 text-white rounded-lg font-semibold text-base hover:bg-purple-700 transition-all duration-200 shadow-md"
      ...
      {showCamera && (
        <CameraCapture
          ...
          onSaveAllComplete={(savedCount) => {
            if (savedCount > 0) {
              // Auto-move to next step after photos are saved
              setTimeout(() => {
                if (onComplete) {
                  onComplete(true);
                }
