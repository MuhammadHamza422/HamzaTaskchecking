...
const QRScanResultPage = () => {
    ...
    return (
      <>
        <Card size="small" className="mb-4">
          <Descriptions title="Box Information" column={{ xs: 1, sm: 2 }} size="small">
        ...
        <Card size="small" className="mb-4">
          ...
          <Row gutter={[16, 16]}>
            ...
            <Col xs={12} sm={6}>
              <div className="text-center p-3 bg-gray-50 rounded">
                <div className="text-xl sm:text-2xl font-bold">{summary?.totalQuantity || 0}</div>
        ...
        <Card size="small">
          ...
          <div className="sm:hidden space-y-3 mt-2">
            {items?.map((item, index) => (
              <Card key={`${item.productId || item.kitId}-${index}`} size="small" className="shadow-sm">
                <div className="flex items-start justify-between mb-3">
                  <Tag color={item.type === "kit" ? "purple" : "blue"}>
                    {item.type === "kit" ? "Kit" : "Product"}
                  </Tag>
                  <div className="text-right">
                    <div className="font-bold text-base text-gray-900">{formatCurrency(item.totalValue || 0, purchaseOrder?.currency)}</div>
                    <div className="text-xs text-gray-400">Total Value</div>
                  </div>
                </div>
                <div className="font-semibold text-base text-gray-900 mb-3">{item.name || "-"}</div>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <div className="text-gray-400 text-xs mb-1">SKU</div>
                  ...
                  <div>
                    ...
                    <div className="font-medium text-gray-900">{item.uom || "-"}</div>
                  </div>
                  <div>
                    <div className="text-gray-400 text-xs mb-1">Unit Price</div>
                    <div className="font-medium text-gray-900">{formatCurrency(item.unitPrice || 0, purchaseOrder?.currency)}</div>
                  </div>
                </div>
              </Card>
            ))}
  ...
  const renderProductContent = () => {
    ...
    return (
      <>
        <Card size="small" className="mb-4">
          <Descriptions title="Product Information" column={{ xs: 1, sm: 2 }} size="small">
  ...
  const renderKitContent = () => {
    ...
    return (
      <>
        <Card size="small" className="mb-4">
          <Descriptions title="Kit Information" column={{ xs: 1, sm: 2 }} size="small">
        ...
        <Card size="small" className="mb-4">
          ...
          <Row gutter={[16, 16]}>
            <Col xs={12} sm={8}>
              <div className="text-center p-3 bg-gray-50 rounded">
                <div className="text-xl sm:text-2xl font-bold">{summary?.totalComponents || 0}</div>
                <div className="text-xs sm:text-sm text-gray-600">Components</div>
              </div>
            </Col>
            <Col xs={12} sm={8}>
              <div className="text-center p-3 bg-gray-50 rounded">
                ...
                <div className="text-xs sm:text-sm text-gray-600">Total Components Qty</div>
              </div>
            </Col>
            <Col xs={24} sm={8}>
              <div className="text-center p-3 bg-gray-50 rounded">
        ...
        <Card size="small">
          ...
          <div className="sm:hidden space-y-3 mt-2">
            {kitProducts?.map((product, index) => (
              <Card key={`${product.productId}-${index}`} size="small" className="shadow-sm">
                <div className="font-semibold text-base text-gray-900 mb-3">{product.name || "-"}</div>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <div className="text-gray-400 text-xs mb-1">SKU</div>
                  ...
                  <div>
                    <div className="text-gray-400 text-xs mb-1">Unit Price</div>
                    <div className="font-medium text-gray-900">{formatCurrency(product.unitPrice || 0, purchaseOrder?.currency)}</div>
                  </div>
                  <div className="col-span-2">
                    <div className="text-gray-400 text-xs mb-1">Total Price</div>
                    <div className="font-bold text-base text-gray-900">{formatCurrency(product.totalPrice || 0, purchaseOrder?.currency)}</div>
                  </div>
                </div>
              </Card>
            ))}
  ...
  return (
    <div className="p-2 sm:p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
              {type === "box" && <Box size={20} className="text-blue-600 sm:w-6 sm:h-6" />}
              {type === "product" && <Package size={20} className="text-green-600 sm:w-6 sm:h-6" />}
              {type === "kit" && <ShoppingBag size={20} className="text-purple-600 sm:w-6 sm:h-6" />}
            ...
            {purchaseOrder && (
              <Link to={`/procurement/orders/${purchaseOrder._id || poId}`}>
                <Button icon={<ExternalLink size={14} />} size="small" className="w-full sm:w-auto">
                  <span className="hidden sm:inline">View Purchase Order</span>
                  <span className="sm:hidden">View PO</span>
                </Button>
              </Link>
            )}
          </div>
        ...
        {purchaseOrder && (
          <Card size="small" className="mb-4">
            <Descriptions title="Purchase Order Information" column={{ xs: 1, sm: 2, md: 3 }} size="small">
              <Descriptions.Item label="PO Reference">
                {purchaseOrder.reference || "-"}
              </Descriptions.Item>
