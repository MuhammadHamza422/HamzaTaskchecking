...
export default function RequestDetailPage() {
  ...
  const handleOrderUpdate = async (values) => {
    try {
      ...
      const isPurchasedNow = String(values?.status || "").trim() === "Purchased";

      // Cast market_order_num to STRING for backend when Purchased, else clear
      const monRaw = values?.market_order_num;
      const monStr = isPurchasedNow
        ? (monRaw === 0 || monRaw ? String(monRaw).trim() : "")
        : "";

      // Normalize URLs; purchase_link validated only when Purchased
      const purchaseRaw = (values?.purchase_link || "").trim();
      let purchase = purchaseRaw ? ensureHttp(purchaseRaw) : "";
      let listing = ensureHttp(values?.listing_link || "");

      if (isPurchasedNow) {
        if (!purchase) {
          toastWarn("Missing purchase link", 'Required when status is "Purchased".');
          return;
        }
        try {
          const u = new URL(purchase);
        ...
        // Not purchased → allow empty/null on server
        purchase = "";
      }
      ...
      const payload = {
        ...values,
        // Send nulls when not Purchased if your backend prefers null to "" (optional):
        market_order_num: isPurchasedNow ? monStr : null,
        purchase_link: isPurchasedNow ? purchase : null,
        listing_link: listing || null,
      };
  ...
  return (
    <div className="page-container" style={{ padding: 16 }}>
      ...
      <Form form={form} layout="vertical" onFinish={handleOrderUpdate} initialValues={{ status: request?.status }}>
        ...
        <Card title="Sourcer Details" style={{ marginBottom: 16 }}>
          <Row gutter={[16, 12]}>
            <Col xs={24} md={12} lg={8}>
              <Form.Item name="seller_name" label="Seller Name">
                <Input placeholder="e.g., John’s Retro Store" readOnly style={{ background: "#fafafa" }} disabled />
              </Form.Item>
            </Col>
            ...
            <Col xs={24} lg={8}>
              <Form.Item name="listing_link" label="Listing Link">
                <Input placeholder="https://..." readOnly style={{ background: "#fafafa" }} disabled/>
              </Form.Item>
            </Col>
        ...
        <Card title="Update Purchase & Tracking Details">
          <Row gutter={[16, 12]}>
            ...
            {/* Market Order # — required & numeric when Purchased; will be sent as STRING/null */}
            <Col xs={24} md={12} lg={8}>
              <Form.Item
                name="market_order_num"
                label="Market Order #"
                ...
                rules={[
                  ({ getFieldValue }) => ({
                    validator(_, value) {
                      ...
                      // required + numeric when Purchased
                      if (value === 0 || value) {
                        const n = typeof value === "number" ? value : Number(value);
                        if (Number.isFinite(n)) return Promise.resolve();
                        return Promise.reject(new Error("Please enter a valid number."));
            ...
            <Col xs={24} md={12} lg={8}>
              <Form.Item
                ...
                hasFeedback
              >
                <Input placeholder="Required when Purchased" />
              </Form.Item>
            </Col>
