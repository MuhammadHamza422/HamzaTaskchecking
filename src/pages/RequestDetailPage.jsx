...
const normalizeRequest = (raw = {}) => {
  ...
  return {
    ...
    // >>> NEW: bring purchaser_remarks into client state <<<
    purchaser_remarks: raw.purchaser_remarks ?? raw.purchaserRemarks ?? "",
...
const TRACKING_STATUSES = ["Pending", "In Transit", "Delivered"];
...
export default function RequestDetailPage() {
  ...
  const handleOrderUpdate = async (values) => {
    try {
      ...
      const trackingLinkCleared =
        trackingLinkProvided &&
        (values.tracking_link == null || values.tracking_link === "");
  ...
  const itemColumns = [
    ...
    },
    {
      title: colTitle(
        "Tested",
        "Whether the item has been functionally tested."
      ),
      dataIndex: "tested",
      key: "tested",
      width: 110,
...

    {
      title: colTitle(
        "Target Cost / Unit",
        "Target price per unit you planned to pay."
      ),
    ...
    // savings columns
    {
      title: colTitle(
        "Savings / Unit",
        <>
          Target Cost / Unit − Actual Cost / Unit.
          <br />
          Positive = you saved per unit; negative = over target.
        </>
      ),
      ...
      render: (_, rec) => {
        ...
        const apu =
          rec.actual_cost_per_unit != null
            ? safeNum(rec.actual_cost_per_unit)
            : safeNum(rec.sellers_price_per_unit);
        const per = tpu - apu;
    ...
    },
    {
      title: colTitle(
        "Savings / SKU",
        <>
          Qty × (Target Cost / Unit − Actual Cost / Unit).
          <br />
          Positive = under target for this SKU.
        </>
      ),
      key: "savings_per_sku",
      align: "right",
...

    {
      title: colTitle(
        "Seller Price / Unit",
        "Allocated portion of seller payment per unit (if available)."
      ),
    ...
    },
    {
      title: colTitle(
        "Actual Cost / Unit",
        "What you actually paid per unit (including allocations)."
      ),
        ...
        <Button
          ...
          onClick={async () => {
            ...
            const res = await Swal.fire({
              ...
              input: "textarea",
              inputLabel: "Reason for removing this product",
              inputPlaceholder: "Type your reason…",
              inputAttributes: { "aria-label": "Reason", maxlength: 1000 },
              inputValidator: (val) =>
                !val || !val.trim() ? "Please enter a reason." : undefined,
            ...
            try {
              ...
              // Compose an audit-friendly line to append
              const line = `• ${new Date().toLocaleString()} — Removed "${
                record.product_name || "Item"
              }"${record.sku ? ` (SKU ${record.sku})` : ""} x${
                record.quantity_needed ?? 1
              }. Reason: ${reason}`;

              const nextRemarks = [request?.purchaser_remarks?.trim(), line]
                .filter(Boolean)
                .join("\n");

              // Do both: delete item + append purchaser_remarks
              await Promise.all([
                apiClient.delete(`/api/v1/sourcing/items/${itemId}`),
                apiClient.patch(`/api/v1/sourcing/${docId}`, {
                  purchaser_remarks: nextRemarks,
                }),
              ]);
              ...
              setRequest((prev) => {
                ...
                return { ...prev, items, purchaser_remarks: nextRemarks };
              });
  ...
  return (
    <div className="page-container" style={{ padding: screens.xs ? 12 : 16 }}>
      ...
      <div className="mb-3">
        <Row
          gutter={[16, 12]}
          // keep everything on one line; allow horizontal scroll on small screens
        ...
        >
          ...
          <Col flex="0 0 280px">
            <StatTile
              ...
              value={
                <span style={{ color: savingsColor, fontWeight: 700 }}>
                  {moneyUSD(totalSavings)}
                </span>
              }
            />
          </Col>
          ...
          <Col flex="0 0 280px">
            <StatTile
              ...
              tip={
                <>
                  Percentage alternative of savings: (Total Savings ÷ Total
                  Target Cost) × 100 = (1 − Actual/Target) × 100.
                </>
              }
              value={
                effPct == null ? (
                  ...
                  <span style={{ color: effColor, fontWeight: 700 }}>
                    {formatPct(effPct)}
                  </span>
                )
              }
          ...
          <Row gutter={gutter}>
            ...
            <Col xs={24} md={12} lg={8}>
              <Form.Item
                ...
                label="Seller Name"
                tooltip={{
                  title: "Choose an existing seller or create a new one.",
                  ...tipCommon,
                }}
              >
                <Select
            ...
            <Col xs={24} md={12} lg={8}>
              <Form.Item
                ...
                label="Marketplace"
                tooltip={{
                  title: "Pick an existing marketplace or create one.",
                  ...tipCommon,
                }}
              >
                <Select
            ...
            <Col xs={24} md={8}>
              <Form.Item
                ...
                label="Shipping"
                tooltip={{
                  title: "Order-level shipping charges (not per-unit).",
                  ...tipCommon,
                }}
                rules={[{ type: "number", transform: Number, min: 0 }]}
              >
            ...
            <Col xs={24} md={8}>
              <Form.Item
                ...
                label="Tax"
                tooltip={{
                  title: "Order-level taxes for this purchase.",
                  ...tipCommon,
                }}
                rules={[{ type: "number", transform: Number, min: 0 }]}
              >
                ...
                {({ getFieldValue }) =>
                  getFieldValue("status") === "Offer" ? (
                    <Form.Item
                      ...
                      label="Offer Price"
                      tooltip={{
                        title: "If making an offer, specify the offered total.",
                        ...tipCommon,
                      }}
                      rules={[
                        {
