...
const normalizeRequest = (raw = {}) => {
  ...
  return {
    ...
    // keep passthroughs you might also show/update
...
export default function RequestDetailPage() {
  // ROUTE params: support /requests/:seller and /request/:sourcingId and /requests/:id
  ...
  // Keep these hooks unconditional
  ...
  const fetchRequest = useCallback(async () => {
    ...
    try {
      ...
      if (idParam && /^[0-9a-fA-F]{24}$/.test(idParam)) {
        // Prefer exact id load when it's an ObjectId
        res = await fetchById(idParam);
      } else if (sellerParam) {
        // Latest by seller
        ...
        try {
          ...
          // fallback: generic list filtered by seller
          res = await apiClient.get("/api/v1/sourcing", { params: { seller: sellerName } });
        }
      } else {
        // non-ObjectId id, still try direct endpoint
        res = await fetchById(idParam);
      }
      ...
      // Put only fields that exist in the form:
      form.setFieldsValue({
  ...
  return (
    <div className="page-container" style={{ padding: 16 }}>
      ...
      {/* One vertical form for all editable fields */}
      <Form
        form={form}
        layout="vertical"
          ...
          <Row gutter={[16, 12]}>
            ...
            <Col xs={24} md={12} lg={8}>
              <Form.Item name="market" label="Marketplace">
                <Select placeholder="Select marketplace" allowClear>
